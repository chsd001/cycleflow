...
from module import discriminator, generator_gatedcnn, generator_nvp
from utils import l1_loss, l2_loss, cross_entropy_loss
from datetime import datetime

class CycleFlowGAN(object):

    def __init__(self, batch_size, num_features, frames, discriminator = discriminator, generator = generator_nvp, mode = 'train', log_dir = '.\log'):

        self.num_features = num_features
        self.input_shape = [batch_size, num_features, frames] # [batch_size, num_features, num_frames]
    ...
    def build_model(self):
        ...
        self.generation_B = self.generator(inputs = self.input_A_real, reuse = False, forward=True)

        self.generation_A = self.generator(inputs = self.input_B_real, reuse = True, forward=False)

        self.discrimination_A_fake = self.discriminator(inputs = self.generation_A, reuse = False, scope_name = 'discriminator_A')
        self.discrimination_B_fake = self.discriminator(inputs = self.generation_B, reuse = True, scope_name = 'discriminator_B')
        ...
        # Reserved for test
        self.generation_B_test = self.generator(inputs = self.input_A_test, reuse = True, forward = True)
        self.generation_A_test = self.generator(inputs = self.input_B_test, reuse = True, forward = False)
    ...
    def train(self, input_A, input_B, generator_learning_rate, discriminator_learning_rate):
...
if __name__ == '__main__':
    ...
    model = CycleFlowGAN(batch_size = 1, num_features = 24, frames = 128)
