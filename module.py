...
def generator_nvp(inputs, reuse = False, train = True, forward = True, scope_name = 'generator_nvp'):
    
    
    ##We add a spatial dimension
    if(forward):
        ##We put the features in position 3
        inputs = tf.transpose(inputs, perm = [0, 2, 1], name = 'input_transpose_fwd')
        x =  tf.squeeze(nvp.model_spec(tf.expand_dims(inputs, axis=1), reuse=reuse, train=train)[0])
        return tf.transpose(x, perm = [0, 2, 1], name = 'output_transpose_fwd')
    else:
        inputs = tf.transpose(inputs, perm = [0, 2, 1], name = 'input_transpose_bwd')
        x = tf.squeeze(nvp.inv_model_spec(tf.expand_dims(inputs, axis=1), reuse=reuse, train=train), axis = 1)
        return tf.transpose(x, perm = [0, 2, 1], name = 'output_transpose_bwd')
